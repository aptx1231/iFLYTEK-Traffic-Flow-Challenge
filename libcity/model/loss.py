...
def mse_diff(preds, labels, null_val=np.nan):
    """
    preds: B, T, N, C
    labels: B, T, N, C
    """
    labels[torch.abs(labels) < 1e-4] = 0
    loss1 = F.mse_loss(preds, labels, reduction='mean')

    diff_pred = preds[:, 1:, :, :] - preds[:, :-1, :, :]
    diff_gold = labels[:, 1:, :, :] - labels[:, :-1, :, :]
    loss2 = F.mse_loss(diff_pred, diff_gold, reduction='mean')
