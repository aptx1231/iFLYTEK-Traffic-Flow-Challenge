...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.ind = self.config.get('ind', 0)
        self.graph_type = self.config.get('graph_type', 'geo')
        self.total = self.config.get('total', True)
        self.dtw_topk = self.config.get('dtw_topk', 10)
        ...
        self.cache_file_name = '../xfdata/train_x.npz'
        self.cache_file_folder = './libcity/cache/dataset_cache/'
        ensure_dir(self.cache_file_folder)
        self.data_path = '../user_data/tmp_data/'
        if not os.path.exists(self.data_path):
            raise ValueError("Dataset {} not exist! Please ensure the path "
                             "'../user_data/tmp_data/' exist!".format(self.dataset, self.dataset))
        # 加载数据集的config.json文件
    ...
    def _calculate_adjacency_matrix(self):
        ...
        # self.adj_mx[self.adj_mx > self.weight_adj_epsilon] = 1
    ...
    def _load_cache_train_val_test(self):
        ...
        Returns:
            tuple: tuple contains:
                ...
                y_test: (num_samples, input_length, ..., feature_dim)
        """
        if self.dataset == 'xunfei':
            train_x = np.load('../xfdata/train_x.npz')['train_x']  # (23974, 12, 207, 1)
            train_y = np.load('../xfdata/train_y.npz')['train_y']  # (23974, 12, 207, 1)
        else:
            raise ValueError('Error dataset {}'.format(self.dataset))

        # 测试集y_test没用
        x_test = np.load('../xfdata/test_x.npz')['test_x']  # (10275, 12, 207, 1)
        y_test = np.zeros(x_test.shape, dtype='float32')  # (10275, 12, 207, 1)

        if self.K == 0:
            train_size = int(len(train_x) * 0.8)
            x_train = train_x[:train_size]
            y_train = train_y[:train_size]
            x_val = train_x[train_size:]
            y_val = train_y[train_size:]
        else:
            unit_x_y_size = len(train_x) // self.K  # K次均分
            board = [0]
            for i in range(1, self.K):
                board.append(board[-1] + unit_x_y_size)
            board.append(len(train_x))
            self._logger.info('{} fold board = {}'.format(self.K, board))  # K+1个数在Board中，代表K段的范围

            # val
            x_val, y_val = train_x[board[self.ind]: board[self.ind + 1]], train_y[board[self.ind]: board[self.ind + 1]]
            self._logger.info('val ' + str(board[self.ind]) + ' : ' + str(board[self.ind + 1]))
            # train
            x_train, y_train = [], []
            for i in range(self.K):
                if i == self.ind:
                    continue
                self._logger.info('train ' + str(board[i]) + ' : ' + str(board[i + 1]))
                x_i = train_x[board[i]: board[i + 1]]
                y_i = train_y[board[i]: board[i + 1]]
                x_train.append(x_i)
                y_train.append(y_i)
            x_train = np.concatenate(x_train, axis=0)  # (b, t, n, f)
            y_train = np.concatenate(y_train, axis=0)  # (b, t, n, f)
    ...
    def _cal_dtw(self, train_data):
        # train_data: (b, t, n, f)
        print(train_data.shape)
        print("dtw_graph_top{}_{}_{}.npy".format(self.dtw_topk, self.ind, self.K))

        if os.path.exists("dtw_graph_top{}_{}_{}.npy".format(self.dtw_topk, self.ind, self.K)):
            graph = np.load("dtw_graph_top{}_{}_{}.npy".format(self.dtw_topk, self.ind, self.K))
            self._logger.info(f"dtw graph links: {graph.sum()}")
            return graph
        elif os.path.exists("dtw_graph_{}_{}.npy".format(self.ind, self.K)):
            dtw_distance = np.load("dtw_graph_{}_{}.npy".format(self.ind, self.K))
            ...
            for i in range(ind.shape[0]):
                for j in range(ind.shape[1]):
                    graph[i][ind[i][j]] = 1
                    graph[ind[i][j]][i] = 1
            np.save("dtw_graph_top{}_{}_{}.npy".format(self.dtw_topk, self.ind, self.K), graph)
            self._logger.info(f"dtw graph links: {graph.sum()}")
            return graph

        origin_train_data = []
        for i in range(train_data.shape[0] - 1):  # 每一条数据取第一步的结果
            origin_train_data.append(train_data[i, 0, :, :])  # (n, f)
        for i in range(train_data[-1].shape[0]):  # 最后一条数据取所有时间步的结果
            origin_train_data.append(train_data[-1, i, :, :])  # (n, f)
        print(len(origin_train_data), origin_train_data[0].shape)
        origin_train_data = np.stack(origin_train_data)
        print('origin_train_data', origin_train_data.shape)  # (t, n, f)

        df = origin_train_data[:, :, -1]  # (t, n) 训练集的Patv
        df = np.swapaxes(df, 0, 1)  # (n, t) 训练集的Patv
        print('df', df.shape)

        self.unit_size = 24 * 12
        data_mean = np.mean(
        ...
        np.save("dtw_graph_{}_{}.npy".format(self.ind, self.K), dtw_distance)
    ...
    def get_data(self):
        ...
        # dtw矩阵
        if self.graph_type == 'dtw':
            self.adj_mx = self._cal_dtw(x_train).astype(np.float32)
            self._logger.info(f"dtw graph links: {self.adj_mx.sum()}")
        elif self.graph_type == 'geo_dtw':
            dtw = self._cal_dtw(x_train).astype(np.float32)
            self.adj_mx[self.adj_mx > self.weight_adj_epsilon] = 1
            self._logger.info(f"geo graph links: {self.adj_mx.sum()}")
            self.adj_mx = np.bitwise_or(self.adj_mx.astype(np.int32), dtw.astype(np.int32)).astype(np.float32)
            self._logger.info(f"geo_dtw graph links: {self.adj_mx.sum()}")
        elif self.graph_type == 'geo':
            self._logger.info(f"geo graph links: {self.adj_mx.sum()}")
        # 数据归一化
