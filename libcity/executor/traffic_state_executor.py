...
class TrafficStateExecutor(AbstractExecutor):
    ...
    def load_model(self, cache_name):
        ...
        model_state = torch.load(cache_name, map_location='cpu')
    ...
    def evaluate(self, test_dataloader):
        ...
        with torch.no_grad():
            ...
            np.save(os.path.join('../user_data/tmp_data', '{}_y_pred.npy'.format(self.exp_id)), y_preds)
    ...
    def train(self, train_dataloader, eval_dataloader):
        ...
        self._logger.info('Best valid loss = {}'.format(min_val_loss))
        return min_val_loss

    def data_augment(self, X, y, p=0.8, alpha=0.5, beta=0.5):
        """
        ...
        random_values = torch.rand([batch_size]).to(self.device)
        ...
        random_betas = torch.FloatTensor(np_betas).to(self.device).reshape([-1, 1, 1, 1])
        index_permute = torch.randperm(batch_size).to(self.device)
